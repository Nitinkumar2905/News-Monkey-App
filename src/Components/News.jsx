...
export class News extends Component {
  ...
  async componentDidMount(){
    let url = `https://newsapi.org/v2/top-headlines?country=in&apiKey=f7fee7c1c75541a7a60f21f14952338f&page=1&pageSize=${this.props.pageSize}`;
  ...
  handleNextClick=async()=>{
    if(this.state.page + 1 > Math.ceil(this.state.totalResults/this.props.pageSize)){
      ...
      let url = `https://newsapi.org/v2/top-headlines?country=in&apiKey=f7fee7c1c75541a7a60f21f14952338f&page=${this.state.page + 1}&pageSize=${this.props.pageSize}`;
      this.setState({loading : true})
      let data = await fetch(url)
      let parsedData = await data.json()
      console.log(parsedData)
